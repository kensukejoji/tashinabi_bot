...
数字を使って視覚的に分かりやすく、「へぇ〜」「知らなかった！」と思えるような驚きのある情報を提供してください。""",
    ...
    "youtube": """【YouTube動画宣伝型】
これは自分のYouTubeチャンネルの動画を宣伝するSNS投稿です。一人称（「私が」「解説しました」「動画にしました」）で書いてください。
動画タイトル：{video_title}
この動画の内容をInstagram・Xのフォロワーに向けて「ぜひ見てほしい」と伝える投稿を作成してください。
「〇〇さんの動画が～」のような他人目線は絶対に使わないでください。
「詳しくは動画で解説しています！」「チャンネルで紹介しました」など自分の動画として紹介してください。
...
【重要】JSON文字列内でダブルクォート（"）を絶対に使わないでください。引用や強調には「」（鉤括弧）を使ってください。
"""
